...
//Variáveis para as funções de correção
double b = 3,
       A = b-1,
       xf = 20,                        //porcentagem de erro na qual a estabilidade ocorre
       criterio = 0.8,                 //criterio para considerar estável
       k = -(1/xf)*log(1-criterio),
       tolBoost = 1,                   //Tolerância de altura para aplicar o boost em Z [m]
       hmax = 0;
float m = 50;
float amp = 2;
double Kx;
...
//////////////////////////////////////////////////////////// INICIO FUNÇÃO CORREÇÃO //////////////////////////////////////////////

double Fator_Correcao(int erro)
{

    double resp = b - (b-1)*exp(-k*erro);
    return abs(resp);
...
double BoostZ(int h)
{
    ...
    {
        double resp = m*(b - (b-1)*exp(-k*(h-tolBoost)));
        return abs(resp);
    }
}
...
//            /vVelInertial.setValue(0,0,offsetZ);
         ...
         if(eye.getPoseAlvar().id == 1){
            ...
            vVel.setValue(Kx*Vx*gainX*prcentVX,Ky*Vy*gainY*prcentVY,0); // Tirando o Z da transformacao

            ros::spinOnce();
            TH_V_I = quadrotor.getHTmatrix(HT_V_I); //obtem mat trans. homogenea do vant em relacao ao inercial

            vVelInertial = tf::quatRotate(TH_V_I.getRotation(), vVel);
            vVelInertial.setZ(Kz*Vz*gainZ*prcentVZ+offsetZ);
        }
